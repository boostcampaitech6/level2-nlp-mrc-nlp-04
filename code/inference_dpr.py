...
from retrieval_dpr import DenseRetrieval, BertEncoder
...
def main():
    ...
    upper_dir = '/'.join(data_args.dataset_name.split('/')[:-2])
    train_path = os.path.join(upper_dir, 'train_dataset')
    train_datasets = load_from_disk(train_path)
    full_ds = concatenate_datasets(
        [
            train_datasets["train"].flatten_indices(),
            train_datasets["validation"].flatten_indices(),
        ]
    )
